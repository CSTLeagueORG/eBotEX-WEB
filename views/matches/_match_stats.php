		...
		<tr>
							...
							<div class="progress" style="width: 450px;">
								<?php foreach($data as $d): ?>
									<?php if($d["type"] == "seperator"): ?>
										<div class="progress-bar progress-bar-warning needTips"
										     title="<?= Yii::t('app', "Halftime"); ?>"
										     style="width: <?php echo $size * 1; ?>px;">
										</div>
									<?php else: ?>
			...
			<td width="50%" valign="top">
				<table class="table table-striped table-condensed">
					<tr>
						<th width="200"></th>
						<th><?php echo $match->teamA? $match->teamA->name : $match->team_a_name; ?> <?php if($match->score_a > $match->score_b): ?>
								<i class="fa fa-star"></i><?php endif; ?></th>
						<th><?php echo $match->teamB? $match->teamB->name : $match->team_b_name; ?> <?php if($match->score_b > $match->score_a): ?>
								<i class="fa fa-star"></i><?php endif; ?></th>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Victory: Bomb Exploded"); ?></th>
						<td><?php echo $stats["team_a"]["bomb_planted_win"]; ?> <?php if($stats["team_a"]["bomb_planted_win"] > $stats["team_b"]["bomb_planted_win"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["bomb_planted_win"]; ?> <?php if($stats["team_b"]["bomb_planted_win"] > $stats["team_a"]["bomb_planted_win"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Defeat: Bomb Defused"); ?></th>
						<td><?php echo $stats["team_a"]["bomb_planted_loose"]; ?> <?php if($stats["team_a"]["bomb_planted_loose"] > $stats["team_b"]["bomb_planted_loose"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["bomb_planted_loose"]; ?> <?php if($stats["team_b"]["bomb_planted_loose"] > $stats["team_a"]["bomb_planted_loose"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Victory: Bomb Defused"); ?></th>
						<td><?php echo $stats["team_a"]["bomb_defused"]; ?> <?php if($stats["team_a"]["bomb_defused"] > $stats["team_b"]["bomb_defused"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["bomb_defused"]; ?> <?php if($stats["team_b"]["bomb_defused"] > $stats["team_a"]["bomb_defused"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Victory"); ?></th>
						<td><?php echo $stats["team_a"]["bomb_explosed"]; ?> <?php if($stats["team_a"]["bomb_explosed"] > $stats["team_b"]["bomb_explosed"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["bomb_explosed"]; ?> <?php if($stats["team_b"]["bomb_explosed"] > $stats["team_a"]["bomb_explosed"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Total Elimination"); ?></th>
						<td><?php echo $stats["team_a"]["kill"]; ?> <?php if($stats["team_a"]["kill"] > $stats["team_b"]["kill"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["kill"]; ?> <?php if($stats["team_b"]["kill"] > $stats["team_a"]["kill"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Victory: Out of Time"); ?></th>
						<td><?php echo $stats["team_a"]["time"]; ?> <?php if($stats["team_a"]["time"] > $stats["team_b"]["time"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
						<td><?php echo $stats["team_b"]["time"]; ?> <?php if($stats["team_b"]["time"] > $stats["team_a"]["time"]): ?>
								<i class="fa fa-star"></i><?php endif; ?></td>
					</tr>
					<tr>
						<th width="200"><?= Yii::t('app', "Longest Round Streak"); ?></th>
						<td>
							<?php echo $round_a; ?> <?php if($round_a > $round_b): ?><i
									class="fa fa-star"></i><?php endif; ?>
						</td>
						<td>
							<?php echo $round_b; ?> <?php if($round_b > $round_a): ?><i
									class="fa fa-star"></i><?php endif; ?>
						</td>
					</tr>
				</table>
									...
									<td valign="top" width="50%">
										...
										<table class="table">
											...
											<tr>
												...
												<td>
													<span class="label label-<?= ($round->bomb_planted)? "success" : "danger" ?>"><i
																class="fa fa-flag"></i></span></td>
											</tr>
											<tr>
												<th width="200"><?= Yii::t('app', "Bombe defused"); ?></th>
												<td>
													<span class="label label-<?= ($round->bomb_defused)? "success" : "danger" ?>"><i
																class="fa fa-flag"></i></span></td>
											</tr>
											<tr>
												<th width="200"><?= Yii::t('app', "Bombe exploded"); ?></th>
												<td>
													<span class="label label-<?= ($round->bomb_exploded)? "success" : "danger" ?>"><i
																class="fa fa-flag"></i></span></td>
											</tr>
										...
										<table class="table table-striped table-condensed">
											<?php foreach(PlayerKill::find()->where(["map_id" => $match->currentMap->id])->andWhere(['round_id' => $round->round_id])->all() as $kill): ?>
												<tr>
													...
													<td width="100">
														<?= Html::img(Yii::$app->urlManager->createAbsoluteUrl("images/kills/csgo/" . $kill->weapon . ".png"), array(
															"class" => "needTips", "title" => $kill->weapon,
														)); ?>
			...
			<tr>
				<td align="center">
					<ul class="pagination" id="paginatorRound">
						<?php foreach($rounds as $round): ?>
							<li><a href="#"
							       onclick="goToRound(<?php echo $round->round_id; ?>); return false;"><?php echo $round->round_id; ?></a>
							</li>
						<?php endforeach; ?>
						<li><a href="#" onclick="return pauseResume();">&nbsp;<i class="fa fa-pause"
						                                                         id="buttonPauseResume"></i></a></li>
					</ul>
				</td>
			</tr>
		</table>
